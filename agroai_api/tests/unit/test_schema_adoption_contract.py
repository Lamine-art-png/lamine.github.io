...
def test_head_contract_covers_security_assurance_platform_field_launch_and_intelligence_memory():
    assert HEAD_ALEMBIC_REVISION == "033_intelligence_wallet_commerce"
    ...
    assert {"passport_id", "action", "created_at"}.issubset(HEAD_SCHEMA_REQUIREMENTS["assurance_review_events"])
    assert {"passport_id", "event_type", "created_at"}.issubset(HEAD_SCHEMA_REQUIREMENTS["assurance_audit_events"])

    assert {"scope_key", "revision", "state_json", "unknowns_json", "conflicts_json", "state_hash"}.issubset(
        HEAD_SCHEMA_REQUIREMENTS["field_states"]
    )
    assert {"field_state_id", "revision", "state_hash", "previous_revision_hash", "evidence_ids_json"}.issubset(
        HEAD_SCHEMA_REQUIREMENTS["field_state_revisions"]
    )
    assert {"field_state_revision_id", "evidence_graph_json", "science_trace_json", "decision_json", "snapshot_hash", "idempotency_key"}.issubset(
        HEAD_SCHEMA_REQUIREMENTS["decision_snapshots"]
    )
    assert {"decision_snapshot_id", "state", "version", "requires_human_approval", "verification_status", "outcome"}.issubset(
        HEAD_SCHEMA_REQUIREMENTS["decision_lifecycles"]
    )
    assert {"lifecycle_id", "sequence", "from_state", "to_state", "event_type", "actor_type", "idempotency_key"}.issubset(
        HEAD_SCHEMA_REQUIREMENTS["decision_lifecycle_events"]
    )
