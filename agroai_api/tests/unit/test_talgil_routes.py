...
class _StubTalgilSensorsRaisesAdapter(_StatusStubAdapter):
    def __init__(self):
        super().__init__(configured=True, status="configured", live=True, targets=1)
...
def test_talgil_target_not_found(monkeypatch):
    ...
    client = TestClient(app)
    assert client.get("/v1/talgil/targets/9999").status_code == 404
...
def test_talgil_status_diagnostics_429_retry_after(monkeypatch):
    adapter = _StatusStubAdapter(
        ...
        live=False,
        targets=0,
        diagnostic=_Diag("TalgilRateLimitError", "Talgil upstream rate limit reached", 429, "Too Many Requests", "text", 120),
    )
    monkeypatch.setattr(AdapterRegistry, "get_talgil", lambda: adapter)
...
def test_talgil_sensors_429_returns_rate_limited(monkeypatch):
    adapter = _StatusStubAdapter(
        ...
        live=False,
        targets=0,
        diagnostic=_Diag("TalgilRateLimitError", "Talgil upstream rate limit reached", 429, retry_after=45),
    )
    monkeypatch.setattr(AdapterRegistry, "get_talgil", lambda: adapter)
...
def test_talgil_status_diagnostics_wrong_key_401(monkeypatch):
    monkeypatch.setattr(
        AdapterRegistry,
        "get_talgil",
        lambda: _StubTalgilConfiguredFailureAdapter("TalgilAuthError", "Talgil auth failed (GET /mytargets)", 401, "Unauthorized", "text"),
    )
    payload = TestClient(app).get("/v1/talgil/status").json()
    assert payload["status"] == "configured"
...
def test_talgil_status_diagnostics_404(monkeypatch):
    monkeypatch.setattr(
        AdapterRegistry,
        "get_talgil",
        lambda: _StubTalgilConfiguredFailureAdapter("TalgilNotFound", "Talgil endpoint not found (GET /mytargets)", 404, "Not Found", "text"),
    )
    payload = TestClient(app).get("/v1/talgil/status").json()
...
def test_talgil_status_diagnostics_non_json_response(monkeypatch):
    monkeypatch.setattr(
        AdapterRegistry,
        "get_talgil",
        lambda: _StubTalgilConfiguredFailureAdapter("TalgilResponseError", "Talgil returned invalid JSON (GET /mytargets)", 200, "<html>bad gateway</html>", "invalid_json"),
    )
    payload = TestClient(app).get("/v1/talgil/status").json()
...
def test_talgil_status_diagnostics_empty_body(monkeypatch):
    monkeypatch.setattr(
        AdapterRegistry,
        "get_talgil",
        lambda: _StubTalgilConfiguredFailureAdapter("TalgilResponseEmpty", "Talgil returned empty payload", 200, "", "empty"),
    )
    payload = TestClient(app).get("/v1/talgil/status").json()
