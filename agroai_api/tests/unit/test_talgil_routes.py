...
class _StubTalgilSensorsRaisesAdapter(_StatusStubAdapter):
    def __init__(self):
        super().__init__(
            ...
            live=False,
            targets=0,
            diagnostic=_Diag("TalgilAuthError", "Talgil auth failed (GET /mytargets)", 401, "denied", "text"),
        )
...
def test_talgil_target_not_found(monkeypatch):
    monkeypatch.setattr(AdapterRegistry, "get_talgil", lambda: _StatusStubAdapter(configured=True, status="live", live=True, targets=1))
    response = TestClient(app).get("/v1/talgil/targets/9999")
    assert response.status_code == 404


def test_talgil_status_missing_credentials(monkeypatch):
    adapter = _StatusStubAdapter(configured=False, status="integration_ready", live=False, targets=0, diagnostic=_Diag("MissingApiKey", "TALGIL_API_KEY is not configured in this runtime."))
    monkeypatch.setattr(AdapterRegistry, "get_talgil", lambda: adapter)
    payload = TestClient(app).get("/v1/talgil/status").json()
...
def test_talgil_status_diagnostics_429_retry_after(monkeypatch):
    adapter = _StatusStubAdapter(configured=True, status="configured", live=False, targets=0, diagnostic=_Diag("TalgilRateLimitError", "Talgil upstream rate limit reached", status_code=429, preview="Too Many Requests", shape="text", retry_after=120))
    monkeypatch.setattr(AdapterRegistry, "get_talgil", lambda: adapter)
    payload = TestClient(app).get("/v1/talgil/status").json()
...
def test_talgil_sensors_429_returns_rate_limited(monkeypatch):
    adapter = _StatusStubAdapter(configured=True, status="configured", live=False, targets=0, diagnostic=_Diag("TalgilRateLimitError", "Talgil upstream rate limit reached", status_code=429, retry_after=45))
    monkeypatch.setattr(AdapterRegistry, "get_talgil", lambda: adapter)
...
def test_talgil_status_diagnostics_wrong_key_401(monkeypatch):
    monkeypatch.setattr(AdapterRegistry, "get_talgil", lambda: _StubTalgilConfiguredFailureAdapter("TalgilAuthError", "Talgil auth failed (GET /mytargets)", status_code=401, preview="Unauthorized", shape="text"))
    payload = TestClient(app).get("/v1/talgil/status").json()
    assert payload["status"] == "configured"
...
def test_talgil_status_diagnostics_non_json_response(monkeypatch):
    monkeypatch.setattr(AdapterRegistry, "get_talgil", lambda: _StubTalgilConfiguredFailureAdapter("TalgilResponseError", "Talgil returned invalid JSON (GET /mytargets)", status_code=200, preview="<html>bad gateway</html>", shape="invalid_json"))
    payload = TestClient(app).get("/v1/talgil/status").json()
...
def test_talgil_status_does_not_call_sensors(monkeypatch):
    adapter = _StatusStubAdapter(configured=True, status="live", live=True, targets=1)
    monkeypatch.setattr(AdapterRegistry, "get_talgil", lambda: adapter)
    response = TestClient(app).get("/v1/talgil/status")
