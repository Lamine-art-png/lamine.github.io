...
def test_qanat_mapping_serializer():
    ...
    assert mapped["parcel"]["parcel_identifier"] == "123-456"
    assert mapped["parcel"]["apn"] is None
...
def test_dynamic_gap_uses_tenant_asset_ids_not_fixture_values(db):
    ...
    db.add(ComplianceParcel(id="parcel-dynamic", tenant_id="tenant-dynamic-gap", parcel_identifier="DYN-1", apn=None))
...
def test_customer_status_hides_research_catalog(db):
    status = services.status_from_repository(_repo(db))
    assert "rule_pack_catalog" not in status
    serialized = str(status)
    assert "research_only_legal_review_required" not in serialized
    assert "us_co_water_rights_research" not in serialized
    assert status["active_rule_pack"]["pack_id"] == "us_ca_sgma_gears"


def test_parcel_identifier_is_global_and_apn_legacy(db):
    repo = _repo(db)
    parcel = repo.parcels()[0]
    assert parcel["parcel_identifier"].startswith("US-CA-SON")
    assert parcel["apn"]


def test_rule_pack_water_budget_thresholds_are_dynamic(db, monkeypatch):
    from app.compliance import rulepacks
    from app.models.compliance import ComplianceWaterBudget

    repo = _repo(db)
    budget = db.query(ComplianceWaterBudget).filter(ComplianceWaterBudget.tenant_id == repo.tenant_id).first()
    budget.projected_balance = 5.0
    db.commit()

    custom_pack = {**rulepacks.CALIFORNIA_PACK, "pack_id": "test_threshold_pack", "workflow_types": ["test_threshold_workflow"], "warning_thresholds": {**rulepacks.CALIFORNIA_PACK["warning_thresholds"], "water_budget_remaining_pct": 30}}
    monkeypatch.setattr(rulepacks, "GLOBAL_PACK_CATALOG", [*rulepacks.GLOBAL_PACK_CATALOG, custom_pack])

    ca = services.readiness_from_repository(repo, "gears_groundwater_extractor_readiness")
    custom = services.readiness_from_repository(repo, "test_threshold_workflow")
    assert not any(w["code"] == "water_budget_threshold_alert" for w in ca["warnings"])
    assert any(w["code"] == "water_budget_threshold_alert" and w["threshold_remaining_pct"] == 30 for w in custom["warnings"])
