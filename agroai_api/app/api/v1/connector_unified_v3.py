...
class UnifiedSelectionRequest(BaseModel):
    resource_ids: list[str] = Field(default_factory=list, max_length=MAX_OPENET_FIELD_IDS)
    scope_mode: ScopeMode = "provider_resources"
    geometry: list[float] = Field(default_factory=list, max_length=MAX_GEOMETRY_NUMBERS)
    field_ids: list[str] = Field(default_factory=list, max_length=MAX_OPENET_FIELD_IDS)
...
async def _probe_candidate(provider: str, api_key: str) -> dict[str, Any]:
    ...
    try:
        ...
        if provider == "openet":
            ...
            return {"identity": {"provider": provider, "account": _redact_mapping(account)}, "resources": []}
...
async def unified_discovery(
    ...
    if result.get("field_ids"):
        config["field_ids"] = _dedupe(list(result["field_ids"]), limit=MAX_OPENET_FIELD_IDS)
    connection.config_json = config
    db.commit()
    db.refresh(connection)
...
async def unified_selection(
    ...
    if connection.provider != "openet":
        ...
        selected = _dedupe(payload.resource_ids, limit=MAX_PROVIDER_RESOURCES)
        if not selected:
        ...
        field_ids = _dedupe(payload.field_ids or payload.resource_ids, limit=MAX_OPENET_FIELD_IDS)
