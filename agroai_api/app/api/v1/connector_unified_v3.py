"""Self-service Connector Unification v3 for WiseConn, Talgil, and OpenET."""
...
ScopeMode = Literal["provider_resources", "agroai_fields", "uploaded_boundaries", "openet_field_ids", "geometry"]
LIFECYCLE_STATES = {
...
MAX_PROVIDER_RESOURCES = 50
MAX_OPENET_FIELDS = 100
MAX_GEOMETRY_NUMBERS = 20_000
...
class UnifiedSelectionRequest(BaseModel):
    resource_ids: list[str] = Field(default_factory=list, max_length=MAX_OPENET_FIELDS)
    scope_mode: ScopeMode = "provider_resources"
    geometry: list[float] = Field(default_factory=list, max_length=MAX_GEOMETRY_NUMBERS)
    field_ids: list[str] = Field(default_factory=list, max_length=MAX_OPENET_FIELDS)
...
def _connection(db: Session, tenant_id: str, connection_id: str) -> ConnectorConnection:
    ...
    if row.provider not in AG_PROVIDERS:
        raise HTTPException(status_code=409, detail={"code": "not_unified_ag_connector"})
    return row
...
def _preserve_or_fail(
    ...
    *,
    ...
    if prior_ref:
        ...
        connection.last_error = f"{message} Existing connection preserved."
    else:
...
async def unified_connect(
    ...
    try:
        probe = await probe_ag_candidate(payload.provider, candidate)
    except AUTH_ERRORS as exc:
        _preserve_or_fail(
            db, tenant_id=tenant_id, connection_id=connection.id,
            prior_status=prior_status, prior_ref=prior_ref,
            failure_status="action_required",
            message="Provider authorization failed. Check the credential and try again.",
        )
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail={"code": "provider_authorization_failed", "provider": payload.provider},
        ) from exc
...
async def unified_discovery(
    ...
    prior_status = connection.status
    ...
    connection = _connection(db, tenant_id, connection_id)
    connection.status = prior_status if prior_status in {"connected", "synced"} else "connected"
    connection.last_error = None
    connection.updated_at = datetime.utcnow()
    config = dict(connection.config_json or {})
    config["discovered_resource_count"] = int(result.get("count", 0))
    if result.get("field_ids"):
        config["field_ids"] = _dedupe(list(result["field_ids"]), MAX_OPENET_FIELDS)
    connection.config_json = config
    db.commit()
    db.refresh(connection)
...
def unified_selection(
    ...
    if connection.provider in {"wiseconn", "talgil"}:
        if payload.scope_mode != "provider_resources":
            raise HTTPException(status_code=409, detail={"code": "invalid_scope_mode_for_provider"})
        selected = _dedupe(payload.resource_ids, MAX_PROVIDER_RESOURCES)
        if not selected:
            raise HTTPException(status_code=422, detail={"code": "provider_resource_selection_required"})
        config.update({
            "scope_mode": "provider_resources",
            "selected_resource_ids": selected,
            "selection_confirmed": True,
        })
    else:
        if payload.scope_mode == "provider_resources":
            raise HTTPException(status_code=409, detail={"code": "invalid_scope_mode_for_openet"})
        field_ids = _dedupe(payload.field_ids or payload.resource_ids, MAX_OPENET_FIELDS)
        if payload.scope_mode == "openet_field_ids" and not field_ids:
            raise HTTPException(status_code=422, detail={"code": "openet_field_ids_required"})
        if payload.scope_mode == "geometry" and len(payload.geometry) < 6:
            raise HTTPException(status_code=422, detail={"code": "openet_geometry_required"})
        if payload.scope_mode == "uploaded_boundaries" and not config.get("openet_asset_id"):
            raise HTTPException(status_code=422, detail={"code": "openet_boundary_asset_required"})
        config.update({
            "scope_mode": payload.scope_mode,
            "selected_resource_ids": field_ids,
            "field_ids": field_ids if field_ids else config.get("field_ids", []),
            "geometry": [float(value) for value in payload.geometry] if payload.geometry else config.get("geometry", []),
            "selection_confirmed": True,
        })

    config["selection_updated_at"] = datetime.utcnow().isoformat() + "Z"
    connection.config_json = sanitize_config(config)
    connection.updated_at = datetime.utcnow()
    db.commit()
    db.refresh(connection)
    return {"status": "ok", **_status_payload(connection)}
...
async def upload_openet_boundary(
    ...
    if connection.provider != "openet":
        raise HTTPException(status_code=409, detail={"code": "openet_only"})
    ...
    if len(data) > max_bytes:
        raise HTTPException(status_code=413, detail={"code": "openet_boundary_too_large"})
    ...
    config = dict(connection.config_json or {})
    config.update({
        "scope_mode": "uploaded_boundaries",
        "openet_asset_id": str(asset_id),
    ...
    db.commit()
    db.refresh(connection)
    return {"status": "ok", "asset_id": str(asset_id), **_status_payload(connection)}
...
async def unified_sync(
    ...
    config = dict(connection.config_json or {})
    if not config.get("selection_confirmed"):
        raise HTTPException(status_code=409, detail={"code": "connector_scope_required", "provider": connection.provider})
    if connection.provider in {"wiseconn", "talgil"} and not config.get("selected_resource_ids"):
        raise HTTPException(status_code=409, detail={"code": "provider_resource_selection_required"})
