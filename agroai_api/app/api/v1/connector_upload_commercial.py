"""Commercially authoritative file-ingestion route.

All file-based connector imports share the evidence_upload quota. This closes the
loophole where manual upload, chat upload, weather files, or another provider
could bypass the monthly commercial capacity by choosing a different card.
"""
...
def _release_if_reserved(db: Session, reservation_id: str | None, reason: str) -> None:
...
async def upload_commercial_evidence_file(
    ...
    try:
        ...
        # ingest_upload currently commits its source/evidence rows. The durable
        # reservation is therefore reloaded and committed immediately after the
        # successful ingest, preserving one usage unit per accepted file import.
        committed_row = db.get(QuotaReservation, reservation_id)
        ...
        commit_reservation(
            ...
            metadata={"provider": provider, "connection_id": connection.id},
        )
        db.commit()
        ...
        db.rollback()
        _release_if_reserved(db, reservation_id, "upload_http_error")
        raise
    except Exception as exc:
        db.rollback()
        _release_if_reserved(db, reservation_id, "upload_ingestion_failed")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail={
                "code": "upload_ingestion_failed",
