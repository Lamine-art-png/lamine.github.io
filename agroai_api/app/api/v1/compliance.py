...
def get_water_budgets(workflow_type: str = Query("gears_groundwater_extractor_readiness"), repo: ComplianceRepository = Depends(get_compliance_repo)) -> list[dict[str, Any]]:
    try:
        return services.readiness_from_repository(repo, workflow_type).get("water_budgets", [])
