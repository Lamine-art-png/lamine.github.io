...
def create_platform_key(
    ...
    *,
    ...
    if project.organization_id != service_account.organization_id or project.id != service_account.api_project_id:
        raise ValueError("service account does not belong to the API project")
    if workspace_id and workspace_id != project.workspace_id:
        workspace = db.get(Workspace, workspace_id)
        if not workspace or workspace.organization_id != project.organization_id:
            raise ValueError("workspace is not available to the project organization")
    ...
    row = PlatformApiKey(
        ...
        workspace_id=workspace_id or service_account.workspace_id or project.workspace_id,
        name=name,
        ...
        cidr_allowlist_json=list(cidr_allowlist or []),
        provider_restrictions_json=dict(provider_restrictions or {}),
        resource_restrictions_json=dict(resource_restrictions or {}),
