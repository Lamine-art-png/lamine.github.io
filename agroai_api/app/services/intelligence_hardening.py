...
a final provenance guard to every customer-visible decision field.
"""
...
from app.services.intelligence_policy import (
    action_requires_human_approval,
    classify_action_kind,
    contains_consequential_action,
    contains_executable_instruction,
)
...
_MEASUREMENT_SOURCE_TYPES = {
    "telemetry", "telemetry_recent", "meter_reading", "sensor", "weather", "weather_observation",
}
_QUALITY_SCORES = {
    "verified": 1.0,
    "accepted": 0.95,
...
def _status_token(value: Any) -> str:
    ...
    aliases = {"notverified": "not_verified", "notvalidated": "not_validated", "needsreview": "needs_review"}
...
def _same_number(left: float | None, right: float | None) -> bool:
    # Grounding serializes deterministic results at bounded precision. This
    # tolerance accepts only that serialization rounding, not a new operating value.
    return left is not None and right is not None and math.isclose(left, right, rel_tol=1e-6, abs_tol=1e-3)
...
def _revalidate_science_checks(packet: IntelligenceGroundingPacket, ineligible_ids: set[str]) -> tuple[list[Any], int]:
    ...
    for check in packet.science_checks:
        ...
        spec_id = check.rule_id
        if check.rule_id == "fao56.etc.single_kc.v1":
            result = registry.run("fao56.etc.single_kc.v1", {"eto_mm": check.inputs.get("eto_mm"), "kc": check.inputs.get("kc")})
            ...
            if isinstance(flow_gpm, (int, float)) and isinstance(runtime, (int, float)):
                ...
                result = registry.run("irrigation.measured_volume.v1", {"flow_m3h": flow_m3h, "runtime_minutes": float(runtime)})
            ...
            if isinstance(gallons, (int, float)) and isinstance(acres, (int, float)):
                spec_id = "irrigation.applied_depth.v1"
                result = registry.run(
                    spec_id,
                    {"volume_m3": float(gallons) * 0.003785411784, "area_ha": float(acres) * 0.40468564224},
                )
...
def enrich_grounding_packet(packet: IntelligenceGroundingPacket, context: EvidenceContext) -> IntelligenceGroundingPacket:
    ...
    packet.grounding_confidence = round(max(0.05, min(packet.grounding_confidence, eligible_score - conflict_penalty)), 3)
...
def _numbers(text: str) -> set[str]:
    """Extract quantities while excluding explicit entity identifiers such as Zone 4."""
    values: set[str] = set()
    source = text or ""
...
def _allowed_operational_numbers(packet: IntelligenceGroundingPacket, question: str) -> set[str]:
    ...
    chunks.extend(
        signal.statement
        for signal in packet.observed_facts
        if signal.statement and _measurement_source(signal.source_type) and signal.provenance.get("operational_eligible") is True
    )
...
def postvalidate_decision(decision: GPT56Decision, packet: IntelligenceGroundingPacket, *, question: str) -> GPT56Decision:
    """Apply an independent fail-closed provenance and action-policy pass."""
    ...
    if removed:
        ...
        decision.risk_flags.append(
            RiskFlag(
                severity="review",
                summary="AGRO-AI removed content that was not traceable to current evidence or a deterministic science result.",
                evidence_ids=[],
            )
        )
    return decision
...
def sanitize_customer_answer(answer: str, packet: IntelligenceGroundingPacket, *, question: str) -> tuple[str, set[str]]:
    """Fail-close free-form recovery output before it reaches a customer."""
    if contains_executable_instruction(answer):
        return (
            "AGRO-AI withheld an operational instruction from a recovery model. "
            "Use the grounded decision path or collect the required evidence before acting.",
