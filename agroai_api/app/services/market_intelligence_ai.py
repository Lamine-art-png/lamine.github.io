...
_DERIVATIVES_INSTRUCTION_RE = re.compile(
    r"\b(?:buy|sell|short|long|enter|open|execute|establish|place|initiate|add|use)\b.{0,100}"
    r"\b(?:futures?|options?|swaps?|derivatives?|hedges?|contracts?)\b"
    r"|\b(?:futures?|options?|swaps?|derivatives?|hedges?)\b.{0,100}"
    r"\b(?:buy|sell|short|long|enter|open|execute|establish|place|initiate|add|use)\b",
    flags=re.IGNORECASE,
)
_ALLOWED_ACTION_KINDS = {"observe", "verify", "reconcile", "compare", "escalate", "collect_data"}
...
def validate_decision_support_policy(payload: dict[str, Any]) -> list[str]:
    """Allow only non-executing decision-support actions and reject unsafe prose."""
    errors: list[str] = []
    ...
    if not isinstance(actions, list):
        ...
        for index, action in enumerate(actions):
            if not isinstance(action, dict):
                errors.append(f"action_not_object:{index}")
                continue
            kind = str(action.get("kind") or "").strip().lower()
            if kind not in _ALLOWED_ACTION_KINDS:
                errors.append(f"disallowed_action_kind:{kind or 'missing'}")
            forbidden_fields = {"instrument", "contract_count", "side", "strike", "expiry", "order_type"}
            if forbidden_fields.intersection(action):
                errors.append(f"trade_parameter_present:{index}")
    visible = " ".join(
        [str(payload.get("summary") or "")]
        + [str(item or "") for item in (payload.get("limitations") or [])]
        + [
    ...
    if _DERIVATIVES_INSTRUCTION_RE.search(visible):
        errors.append("personalized_derivatives_instruction")
    return list(dict.fromkeys(errors))
...
async def generate_market_brief(
    ...
    *,
    ...
    system = (
        ...
        "Never invent or calculate financial numbers. Never describe a scenario as a forecast. Never recommend establishing a hedge, using a "
        "specific number of contracts, or entering any derivatives position. Actions must use only the allowlisted non-executing kinds in the schema. "
    ...
    schema = {
        ...
        "json_schema": {
            ...
            "schema": {
                ...
                "properties": {
                    ...
                    "actions": {
                        ...
                        "items": {
                            "type": "object",
                            "properties": {
                                "kind": {"type": "string", "enum": sorted(_ALLOWED_ACTION_KINDS)},
                ...
                "required": ["summary", "insights", "confidence", "limitations", "actions"],
                "additionalProperties": False,
            },
        },
    }
