...
class RedisTaskQueue:
    def __init__(self, client: Any | None = None):
        ...
        self.stream = os.getenv("TASK_QUEUE_STREAM", "agroai:tasks")
        self.group = os.getenv("TASK_QUEUE_GROUP", "agroai-workers")
    ...
    def enqueue(self, job_id: str, tenant_id: str, task_type: str) -> str:
        self.ensure_group()
        return str(self.client.xadd(self.stream, {"job_id": job_id, "tenant_id": tenant_id, "task_type": task_type}))

    def read(self, consumer: str, *, block_ms: int = 5000) -> list[QueueMessage]:
        self.ensure_group()
        response = self.client.xreadgroup(self.group, consumer, {self.stream: ">"}, count=5, block=block_ms)
    ...
    def claim_stale(self, consumer: str, *, min_idle_ms: int = 120000) -> list[QueueMessage]:
        self.ensure_group()
        response = self.client.xautoclaim(self.stream, self.group, consumer, min_idle_time=min_idle_ms, start_id="0-0", count=10)
