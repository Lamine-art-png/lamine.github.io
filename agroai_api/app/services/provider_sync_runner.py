...
from app.services.provider_oauth import ProviderOAuthError, refresh_provider_credentials, scopes_from_payload, token_expiry
...
def _persist_connection_failure_state(db: Session, *, connection_id: str | None, tenant_id: str, state: str, exc: Exception) -> None:
...
def _retry_with_connection_state(
    ...
    *,
    ...
    if result == "failed":
        _persist_connection_failure_state(db, connection_id=connection_id, tenant_id=tenant_id, state="failed", exc=exc)
    elif result == "retrying":
        _persist_connection_failure_state(db, connection_id=connection_id, tenant_id=tenant_id, state=retry_state, exc=exc)
    return result


async def process_provider_sync_job(db: Session, *, job_id: str, tenant_id: str, worker_id: str) -> str:
    ...
    with job_lease_heartbeat(job_id=job_id, tenant_id=tenant_id, worker_id=worker_id) as heartbeat:
        try:
            ...
            return _reconnect_failure(db, job_id=job_id, exc=ProviderOAuthError(str(exc), reconnect_required=True), worker_id=worker_id)
        except RATE_LIMIT_ERRORS as exc:
            return _retry_with_connection_state(
                db, job_id=job_id, connection_id=connection_id, tenant_id=tenant_id,
                worker_id=worker_id, exc=exc, retry_state="rate_limited",
            )
            ...
            return _retry_with_connection_state(
                db, job_id=job_id, connection_id=connection_id, tenant_id=tenant_id,
                worker_id=worker_id, exc=exc, retry_state="degraded",
            )
