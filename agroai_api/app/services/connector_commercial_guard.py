...
def enforce_connector_write(session: Session, connection: ConnectorConnection, *, check_capacity: bool) -> None:
    ...
    # Network Custom API used to be a preview state. Allowing preview here keeps
    # old persisted plan rows compatible while the v2 packaging installer
    # upgrades the canonical Network/Enterprise state to enabled.
    require_feature(
        session,
        org,
        feature_key,
