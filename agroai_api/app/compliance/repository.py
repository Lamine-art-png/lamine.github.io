...
class ComplianceRepository:
    ...
    def seed_demo_fixture_if_empty(self) -> None:
        ...
        for parcel in VINEYARD_FIXTURE["parcels"]:
            self.db.add(ComplianceParcel(id=parcel["id"], tenant_id=self.tenant_id, parcel_identifier=parcel.get("parcel_identifier") or parcel.get("apn"), apn=parcel.get("apn"), geometry_ref=parcel["geometry_ref"], county=parcel["county"]))
    ...
    def parcels(self) -> list[dict[str, Any]]:
        return [{"id": r.id, "organization_id": r.tenant_id, "parcel_identifier": r.parcel_identifier, "apn": r.apn, "geometry_ref": r.geometry_ref, "county": r.county} for r in self.db.query(ComplianceParcel).filter(ComplianceParcel.tenant_id == self.tenant_id).all()]
