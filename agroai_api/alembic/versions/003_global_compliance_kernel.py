...
def upgrade():
    ...
    if _table_exists("compliance_parcels"):
        _add_column_if_missing("compliance_parcels", sa.Column("parcel_identifier", sa.String(), nullable=True))
        if bind.dialect.name == "postgresql":
            if _has_column("compliance_parcels", "apn"):
                op.execute("UPDATE compliance_parcels SET parcel_identifier = apn WHERE parcel_identifier IS NULL")
            op.alter_column("compliance_parcels", "parcel_identifier", nullable=False)
            op.alter_column("compliance_parcels", "apn", nullable=True)
        else:
            if _has_column("compliance_parcels", "apn"):
                op.execute("UPDATE compliance_parcels SET parcel_identifier = apn WHERE parcel_identifier IS NULL")
    ...
    _create_index_if_missing("ix_compliance_parcels_parcel_identifier", "compliance_parcels", ["parcel_identifier"])
