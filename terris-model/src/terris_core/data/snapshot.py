...
def build_snapshot(dataset: Path, manifest: Path | None, output: Path) -> dict:
    ...
    if not records:
        raise ValueError("Cannot snapshot an empty Terris dataset.")

    domains = sorted({str(r.get("domain", "unknown")) for r in records})
    languages = sorted({str(r.get("language", "unknown")) for r in records})
    customer_rows = [r for r in records if bool(r.get("provenance", {}).get("customer_data"))]
    unconsented = [
        r.get("id", "unknown")
        for r in customer_rows
        if r.get("provenance", {}).get("rights") != "explicit_training_consent"
        or not bool(r.get("provenance", {}).get("training_allowed"))
    ]
    if unconsented:
