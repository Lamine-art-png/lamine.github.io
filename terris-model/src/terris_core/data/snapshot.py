...
def build_snapshot(dataset: Path, manifest: Path | None, output: Path) -> dict:
    ...
    customer_rows = [r for r in records if r.get("provenance", {}).get("customer_data")]
    unconsented = [r.get("id") for r in customer_rows if r.get("provenance", {}).get("rights") != "explicit_training_consent"]
    if unconsented:
