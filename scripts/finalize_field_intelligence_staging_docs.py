...
smoke = smoke.replace("production canary smoke", "deployed-environment smoke", 1)
smoke = replace_once(smoke, "import struct\n", "import struct\nimport zlib\nfrom pathlib import Path\n", "smoke imports")
fixture_pattern = re.compile(r"def _png\(\) -> bytes:.*?(?=\n\ndef main\(\) -> None:)", re.S)
fixture_source = r'''def _png() -> bytes:
...
'''
smoke, substitutions = fixture_pattern.subn(lambda _match: fixture_source, smoke, count=1)
if substitutions != 1:
    raise SystemExit("missing smoke fixture block")
smoke = replace_once(
    smoke,
...
staging_tests = load(staging_test_path)
staging_tests += '''
...
'''
launch_tests = replace_once(
    launch_tests,
    "\n\n# --------------------------------------------------------------------------- #\n# Worker topology\n",
    admin_test + "\n\n# --------------------------------------------------------------------------- #\n# Worker topology\n",
    "launch admin test insertion",
)
save(launch_test_path, launch_tests)
