...
locals {
  # unique per run (CI overrides service_name); avoids log group collisions
  log_group_name = "/aws/ecs/${var.service_name}/api"
}
...
module "api_service" {
  ...
  # One container (nginx), kept in foreground so ECS sees it as healthy
