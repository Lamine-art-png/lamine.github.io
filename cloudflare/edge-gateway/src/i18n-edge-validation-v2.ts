...
export function bearerToken<Host, Cf>(request: Request<Host, Cf>): string {
  const value = request.headers.get("authorization") || "";
  return value.toLowerCase().startsWith("bearer ") ? value.slice(7).trim() : "";
}

export function canaryAuthorized<Host, Cf>(request: Request<Host, Cf>, expected: string): boolean {
...
export function registryRequest<Host, Cf>(request: Request<Host, Cf>): Request {
...
export function englishValidationRequest<Host, Cf>(request: Request<Host, Cf>, source: unknown, hasSource: boolean): Request {
  const headers = new Headers(request.headers);
  headers.delete("content-length");
