...
async function translateChunk(
  ...
  const messages = [
    {
      role: "system",
      content: `Translate every JSON string value from English into ${languageName} (BCP-47 locale ${locale}). Return one JSON object only. Preserve every key exactly. Preserve placeholders in braces exactly. Preserve AGRO-AI, product names, URLs, units, numbers, and Markdown syntax. Translate naturally and concisely for a professional enterprise software interface.`,
    },
...
export async function translateCatalog(
  ...
  for (let index = 0; index < chunks.length; index += MAX_PARALLEL) {
    const results = await Promise.all(
      chunks.slice(index, index + MAX_PARALLEL).map((chunk) => translateChunk(ai, locale, languageName, chunk)),
    );
