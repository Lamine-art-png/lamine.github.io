...
async function accurateModelEvidence(response: Response): Promise<Response> {
  ...
  payload.models = [dedicatedTranslationModel];
...
export default {
  async fetch(request: Request, env: I18nFastpathEnv): Promise<Response> {
    ...
    if (request.method === "POST" && translationPaths.has(pathname)) {
      const realAi = env.AI;
      const adaptedEnv = { ...env, AI: new DedicatedTranslationAdapter(realAi) } as I18nFastpathEnv;
      const response = await handleI18nFastpath(request, adaptedEnv, baseFetch);
      return accurateModelEvidence(response);
    }
