...
function complianceHeaders(): HeadersInit {
  ...
  if (!demoToken) {
    throw new Error("Compliance browser access is disabled until a backend session/proxy is wired. Configure VITE_NON_PRODUCTION_COMPLIANCE_DEMO_TOKEN only for labeled non-production demos.");
  }
