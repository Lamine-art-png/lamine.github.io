...
assert(hook.includes("hasCoreLocaleCatalog"), "core catalog readiness gate missing");
...
assert(hook.includes('ensureLocaleCatalog(selectedLocale, "core")'), "remaining bundled core copy must hydrate after critical shell readiness");
...
assert(criticalHydrationIndex < coreHydrationIndex, "critical hydration must occur before core hydration");
assert(coreHydrationIndex < fullHydrationIndex, "core hydration must occur before full hydration");
const interactiveReadyIndex = hook.indexOf("setCatalogLoading(false);", criticalHydrationIndex);
assert(interactiveReadyIndex > criticalHydrationIndex && interactiveReadyIndex < coreHydrationIndex, "portal interactivity must resume after critical shell hydration and before remaining core/full convergence");
...
assert(dynamicCatalog.includes('normalized.toLowerCase() !== "[object object]"'), "dynamic locale catalogs must reject serialized object sentinel values");
