...
const organizationTypes = [
  ["farm_or_grower", "Farm or grower"], ["agribusiness", "Agribusiness"],
  ["agricultural_landowner", "Agricultural landowner"], ["investment_manager", "Agricultural investment manager"],
  ["irrigation_dealer_or_contractor", "Irrigation dealer or contractor"], ["irrigation_technology_provider", "Irrigation technology provider"],
  ["oem_or_equipment_manufacturer", "OEM or equipment manufacturer"], ["agricultural_consultant", "Agricultural consultant"],
  ["research_institution", "Research institution"], ["water_agency_or_district", "Water agency or district"],
  ["food_or_supply_chain_company", "Food or supply-chain company"], ["other_agricultural_organization", "Other agricultural organization"],
] as const;
...
function VerificationPanel() {
  ...
  async function resend() {
    setWorking("resend");
    try { setMessage(await requestVerification(verification?.email)); } finally { setWorking(""); }
  }
  async function refresh() {
    if (!verification?.email || !password) return;
    setWorking("refresh");
    try { await login(verification.email, password); }
    catch (error) { setMessage(error instanceof Error ? error.message : "Verification is still pending."); }
    finally { setWorking(""); }
  }
...
export function AuthScreen() {
  ...
  const sideBullets = platformHostname ? [
    "Organization verification and Platform API enrollment are separate security decisions.",
    "Approved developers begin with isolated test projects and synthetic data.",
    "Live projects, billing, provider credentials, and physical actions remain separately gated.",
  ] : [
  ...
  return <div className="min-h-screen grid lg:grid-cols-[0.92fr_1.08fr]" style={{ background: "#EEE9DB" }}>
    <section className="relative overflow-hidden px-8 py-8 lg:sticky lg:top-0 lg:h-screen lg:px-12 lg:py-12" style={{ background: platformHostname ? "radial-gradient(circle at 16% 16%,rgba(220,239,139,.22),transparent 27%),radial-gradient(circle at 82% 68%,rgba(81,139,100,.22),transparent 31%),linear-gradient(155deg,#061B13 0%,#0A281C 46%,#123A28 100%)" : "radial-gradient(circle at 18% 22%,rgba(208,231,167,.28),transparent 26%),radial-gradient(circle at 78% 20%,rgba(100,149,115,.26),transparent 28%),linear-gradient(160deg,#071B14 0%,#0E2A1F 38%,#153628 100%)" }}>
      <div className="absolute inset-0 opacity-30" style={{ backgroundImage: "linear-gradient(rgba(255,255,255,.08) 1px,transparent 1px),linear-gradient(90deg,rgba(255,255,255,.08) 1px,transparent 1px)", backgroundSize: platformHostname ? "34px 34px" : "38px 38px", maskImage: "radial-gradient(circle at center,black,transparent 82%)" }} />
      <div className="relative flex h-full flex-col justify-between">
        <div className="flex items-center gap-3"><div className="flex h-11 w-11 items-center justify-center overflow-hidden rounded-xl bg-[#1A4F39] shadow-[0_8px_30px_rgba(0,0,0,.2)]"><ImageWithFallback src={logoImg} alt="AGRO-AI" className="h-full w-full object-contain" /></div><div><div className="text-[15px] font-semibold tracking-tight text-white">AGRO-AI</div><div className="text-[11px] text-white/45">{platformHostname ? "Platform API" : "Enterprise Portal"}</div></div></div>
        <div className="relative max-w-xl py-14"><div className="mb-5 inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-3 py-1 text-[11px] font-medium uppercase tracking-[.18em] text-[#DDEB8F]">{platformHostname ? <Code2 className="h-3.5 w-3.5" /> : <ShieldCheck className="h-3.5 w-3.5" />}{platformHostname ? "Developer private beta" : "Verified agricultural access"}</div><h1 className="max-w-lg text-[42px] font-semibold leading-[1.05] tracking-tight text-white">{platformHostname ? "Build on AGRO-AI." : "AGRO-AI Enterprise Portal"}</h1><p className="mt-5 max-w-xl text-[15px] leading-7 text-white/72">{platformHostname ? "Create a verified agricultural organization account, then apply for bounded test access to the AGRO-AI Platform API." : "Live workspaces are reserved for verified farms, agribusinesses, water organizations, advisors, and agricultural technology partners."}</p><div className="mt-8 grid max-w-lg gap-3">{sideBullets.map((line) => <div key={line} className="rounded-xl border border-white/10 bg-white/5 p-4 text-[13px] leading-6 text-white/78 backdrop-blur-sm">{line}</div>)}</div></div>
        <div className="relative text-[11px] leading-5 text-white/42">{platformHostname ? "Verified account. Reviewed enrollment. Controlled activation." : "Real teams. Real operations. Verified access."}</div>
      </div>
    </section>
...
function Field({ label, children }: { label: string; children: ReactNode }) {
  return <label className="block"><span className="mb-1.5 block text-[12px] font-medium text-[#10231B]">{label}</span>{children}</label>;
}
