...
export function LanguageSelector({ compact = false, dark = false }: { compact?: boolean; dark?: boolean }) {
  ...
  const selected = useMemo(() => LOCALES.find((item) => item.code === locale) || LOCALES[0], [locale]);
    ...
    <label className={`flex ${compact ? "items-center gap-2" : "flex-col gap-1"}`} style={{ color: labelColor }}>
      {!compact ? (
        <span className="inline-flex items-center gap-1 text-[11px] font-medium">
          <Globe2 className="h-3.5 w-3.5" /> {t("language", locale)}
        </span>
      ) : null}
      <span className="relative inline-flex w-full items-center">
        {compact ? <Globe2 className="pointer-events-none absolute left-2 h-3.5 w-3.5" style={{ color: labelColor }} /> : null}
        <span className="pointer-events-none absolute left-7 text-[13px]" aria-hidden="true">{selected.flag}</span>
        <select
          value={locale}
          onChange={(event) => {
          ...
          className="h-9 w-full appearance-none rounded-md py-1 pl-12 pr-8 text-[12px] outline-none"
          style={{ background: selectBg, color: selectColor, border: `1px solid ${border}` }}
          title="Language"
        >
          {LOCALES.map((item) => (
            <option key={item.code} value={item.code}>{item.flag} {item.nativeName} · {item.englishName}</option>
          ))}
        </select>
