...
function linkedFieldObservationEvidence(observation: AnyRecord): AnyRecord {
  ...
  return {
    ...
    parsed_preview: JSON.stringify({
      ...
      recommended_action: safeText(observation.recommended_action) || safeText(vision.recommended_follow_up),
...
function contextualPrompt(observation: AnyRecord): string {
  const summary = safeText(observation.summary)
...
function FieldObservationContext({ controller }: { controller: IntelligenceController }) {
  ...
  return <section data-field-observation-context={observationId} className="border-b border-[#BFD8C9] bg-[#EDF7F1] px-4 py-3 sm:px-8">
    <div className="mx-auto flex max-w-[1100px] flex-wrap items-start justify-between gap-3">
      <div className="min-w-0 flex-1">
        ...
        <div className="mt-1 line-clamp-2 text-[12px] leading-5 text-[#536158]">
          {loadError || safeText(observation?.summary) || safeText(observation?.corrected_transcript) || safeText(observation?.transcript) || "Loading the exact field observation…"}
        </div>
      </div>
