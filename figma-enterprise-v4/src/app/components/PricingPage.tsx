      ...
      <section className="mt-7 grid gap-4 lg:grid-cols-5">
        {plans.map((plan) => {
          ...
          return <article key={plan.id} className="flex min-w-0 min-h-[500px] flex-col overflow-hidden rounded-[22px] p-5" style={{ background: highlighted ? "#0D2B1E" : SURFACE, border: `1px solid ${highlighted ? "#0D2B1E" : BORDER}`, boxShadow: highlighted ? "0 20px 70px rgba(13,43,30,0.18)" : "0 12px 38px rgba(16,35,27,0.06)" }}>
            <div className="min-w-0"><h2 className="text-[21px] font-semibold" style={{ color: highlighted ? "white" : TEXT }}>{plan.name}</h2><CardBadge plan={plan} requestedUpgrade={requestedUpgrade} billingPeriod={billingPeriod} highlighted={highlighted} /><p className="mt-3 min-h-[92px] text-[12px] leading-5" style={{ color: highlighted ? "rgba(255,255,255,0.68)" : MUTED }}>{plan.recommended_buyer}</p></div>
            <PriceDisplay value={planPrice(plan, billingPeriod)} highlighted={highlighted} />
            <div className="mt-4 space-y-2 text-[12px]" style={{ color: highlighted ? "rgba(255,255,255,0.82)" : TEXT }}>{limits.map((limit) => <div key={limit} className="flex gap-2"><Check className="mt-0.5 h-3.5 w-3.5 shrink-0" /><span>{limit}</span></div>)}</div>
            <div className="mt-5 space-y-2">{plan.features.slice(0, 5).map((feature) => <div key={feature} className="flex gap-2 text-[12px] leading-5" style={{ color: highlighted ? "rgba(255,255,255,0.82)" : TEXT }}><Check className="mt-0.5 h-3.5 w-3.5 shrink-0" /><span>{feature}</span></div>)}</div>
            <div className="mt-auto pt-6"><button type="button" disabled={busyPlan === plan.id} onClick={() => choosePlan(plan)} className="h-11 w-full rounded-xl text-[13px] font-semibold disabled:opacity-60" style={{ background: highlighted ? "white" : "#0D2B1E", color: highlighted ? "#0D2B1E" : "white" }}>{busyPlan === plan.id ? "Opening Stripe..." : label}</button></div>
          </article>;
        })}
      </section>

      <section className="mt-10 rounded-[24px] p-5 md:p-7" style={{ background: SURFACE, border: `1px solid ${BORDER}` }}>
        <div className="mb-5 flex items-center gap-2"><ShieldCheck className="h-5 w-5" style={{ color: GREEN }} /><h2 className="text-[24px] font-semibold" style={{ color: TEXT }}>Compare plans</h2></div>
        <div className="overflow-x-auto"><table className="w-full min-w-[900px] border-collapse text-[13px]"><thead><tr style={{ color: MUTED }}><th className="py-3 text-left font-medium">Capability</th>{plans.map((plan) => <th key={plan.id} className="py-3 text-center font-medium">{plan.name}</th>)}</tr></thead><tbody>{COMPARISON.map((row) => <tr key={row[0]} style={{ borderTop: `1px solid ${BORDER}` }}>{row.map((cell, index) => <td key={`${row[0]}-${index}`} className={`px-3 py-4 ${index ? "text-center" : "font-medium"}`} style={{ color: TEXT }}>{cell}</td>)}</tr>)}</tbody></table></div>
      </section>
    </main>
  </div>;
}
