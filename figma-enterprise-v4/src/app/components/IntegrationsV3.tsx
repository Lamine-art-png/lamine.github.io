...
export function IntegrationsV3() {
  ...
  function openConnector(connector: Connector) {
    ...
    if (PLAN_ORDER.indexOf(plan) < PLAN_ORDER.indexOf(needed)) {
      ...
      openCommercialBoundary({ status: 402, code: "upgrade_required", feature: featureFor(connector.id), recommended_plan: needed, message: `${planName(needed)} is required to connect ${profile.title}.`, source: "connectors_v3" });
      return;
    }
