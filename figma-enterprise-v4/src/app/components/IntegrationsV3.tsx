...
export function IntegrationsV3() {
  ...
  const cards = useMemo(() => {
    ...
    Object.keys(PROFILES).forEach((id) => { if (!byId.has(id)) { const profile = PROFILES[id]; byId.set(id, { id, name: profile.title, category: TYPE_LABEL[profile.type], status: "available", required_plan: "internal", connection_methods: [], imports: [], used_by: [], promise: profile.description, upload_supported: profile.supportsUpload }); } });
  ...
  function openConnector(connector: Connector) { const existing = connections.find((row) => row.provider === connector.id) || connector.connection || null; setSelected(connector); setConnection(existing); setMessage(""); }
  ...
  return <div className="min-h-screen" style={{ background: BG }}>
    <header className="px-8 py-7" style={{ background: SURFACE, borderBottom: `1px solid ${BORDER}` }}><div className="flex items-start justify-between gap-6"><div><div className="flex items-center gap-2 mb-3"><StatusBadge label="Integration Hub" tone="good" /><StatusBadge label={`${plan} testing`} /></div><h1 className="text-[30px] font-semibold tracking-tight" style={{ color: TEXT }}>Connectors</h1><p className="mt-2 max-w-3xl text-[14px] leading-relaxed" style={{ color: MUTED }}>Authorize, choose scope, sync, done. Provider complexity stays behind AGRO-AI.</p></div><PortalButton variant="secondary" onClick={refresh}>Refresh</PortalButton></div></header>
