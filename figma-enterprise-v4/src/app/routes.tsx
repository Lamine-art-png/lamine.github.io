...
const INTELLIGENCE_FIRST_PATHS = new Set(["/", "/home", "/api-keys", "/playground", "/usage", "/billing", "/docs"]);

function platformRelativePath() {
  const route = window.location.pathname.replace(/^\/platform(?=\/|$)/, "") || "/";
  return route || "/";
}
...
function PlatformProduct() {
  ...
  const simple = INTELLIGENCE_FIRST_PATHS.has(platformRelativePath());
  return <>{simple ? <PlatformIntelligenceConsole /> : <PlatformConsoleApp />}<PlatformSafetyNotice /></>;
}
...
const operationRoutes = [
  ...
  { path: "market-intelligence", lazy: lazyComponent(() => import("./components/MarketIntelligenceV2"), "MarketIntelligenceV2") },
