...
export const apiClient = {
  ...
  intelligence: { ask: (payload: IntelligenceAskPayload) => post("/v1/intelligence/ask", payload), run: (payload: IntelligenceRunPayload) => post("/v1/intelligence/run", payload), workbench: (payload: WorkbenchRunPayload) => post("/v1/intelligence/workbench", payload), reportFactory: (payload: ReportFactoryPayload) => post("/v1/intelligence/report-factory", payload) },
  connectors: { list: () => get("/v1/connectors"), connections: () => get("/v1/connectors/connections"), start: (payload: ConnectorStartPayload) => post("/v1/connectors/start", payload), connect: (payload: ConnectorConnectPayload) => post("/v1/connectors/connect", payload), sync: (connectionId: string) => post(`/v1/connectors/connections/${encodeURIComponent(connectionId)}/sync`), disconnect: (connectionId: string) => post(`/v1/connectors/connections/${encodeURIComponent(connectionId)}/disconnect`) },
  fieldOps: { tasks: () => get("/v1/field-ops/tasks"), createTask: (payload: FieldOpsTaskPayload) => post("/v1/field-ops/tasks", payload), updateTaskStatus: (taskId: string, payload: FieldOpsTaskStatusPayload) => patch(`/v1/field-ops/tasks/${encodeURIComponent(taskId)}/status`, payload), fieldUpdate: (payload: FieldUpdatePayload) => post("/v1/field-ops/updates", payload), message: (payload: FieldMessagePayload) => post("/v1/field-ops/messages", payload) },
  autopilot: { report: (payload: AutopilotReportPayload) => post("/v1/autopilot/report", payload) },
};
