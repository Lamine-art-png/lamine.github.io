  ...
  return (
    <div className="min-h-screen bg-[#F6F4EE] px-6 py-12 text-[#10231B]">
      <div className="mx-auto max-w-[760px] rounded-2xl border border-[#D6DDD0] bg-[#FFFDF8] p-8 shadow-[0_20px_60px_rgba(16,35,27,0.08)]">
        ...
        <p className="mt-3 text-[14px] leading-7 text-[#65736A]">The portal booted safely, but one workspace route failed to load. This screen prevents a white page while the route is repaired.</p>
        ...
        <div className="mt-6 flex flex-wrap gap-3">
          ...
          <button type="button" onClick={() => { window.localStorage.removeItem("agroai_access_token"); window.location.href = "/"; }} className="rounded-lg border border-[#D6DDD0] bg-white px-4 py-2 text-[13px] font-medium text-[#10231B]">Clear session and sign in again</button>
        </div>
      </div>
    </div>
  );
...
function AuthenticatedApp() {
  ...
  if (isLoading) return <div className="min-h-screen flex items-center justify-center bg-[#F6F4EE] text-[#68776F] text-sm">Loading session</div>;
  ...
  if (!router) return <div className="min-h-screen flex items-center justify-center bg-[#F6F4EE] text-[#68776F] text-sm">Loading portal</div>;
