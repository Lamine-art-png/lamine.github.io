...
export function renderCommandCenter(state) {
  ...
  return `<div class="screen-stack">
    ...
    <section class="hero-panel">
      ...
      <div class="hero-metrics command-kpis">${metricCard("Farm / block", isDemo ? "Alpha Vineyard · Block A North" : "WiseConn zone 162803")}${metricCard(
        "Controller source",
        isDemo ? "WiseConn demo" : "Live WiseConn"
      )}${metricCard("Today’s decision", decision)}${metricCard("Confidence", confidence)}${metricCard("Data quality", dataQuality)}${metricCard("Verification state", isDemo ? "Verification pending" : state.live.recommendation ? "Verification pending" : "Data source pending")}</div>
    </section>
