...
export function renderCommandCenter(state) {
  ...
  <article class="intake-card"><h4>Upload data package</h4><ul><li>controller_events.csv</li><li>weather_summary.csv</li><li>soil_moisture.csv</li><li>field_notes.txt</li></ul><input id="workbench-upload-input" type="file" accept=".csv,.json,.txt,.xlsx" /><button class="button secondary" data-action="load-demo-data-package">Load demo data package</button><small>Demo intake simulation. Production upload requires backend ingestion endpoint.</small></article></div>
  <p class="muted">Selected intake: ${escapeHtml(runtime.intakeModeLabel)}</p><div class="muted">Uploaded artifacts: ${(runtime.analysis.artifacts||[]).map((a)=>`${a.filename} (${a.rows_detected} rows · ${a.source_kind})`).join("; ") || "none"}</div></section>
  ...
  <p class="muted">${escapeHtml(runtime.analysis.statusLabel)}</p><p class="muted">${runtime.analysis.backendResult ? `Confidence ${runtime.analysis.backendResult.reconciliation?.confidence_score} · ${runtime.analysis.backendResult.reconciliation?.evidence_completeness}` : ""}</p>
